...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (resolveWarning) console.warn(`[analyze/video] ${resolveWarning}`);
    console.log(`[analyze/video] Analyzing: ${resolvedUrl} (${effectiveDuration}s${videoMetadata?.platform ? `, ${videoMetadata.platform}` : ''})`);
    ...
    saveAnalysisFrames(supabase, req.user.id, req.user.email, frameUrls, videoMetadata, 'video-analysis')
      .catch(err => console.warn(`[analyze/video] Frame save failed (non-fatal): ${err.message}`));
