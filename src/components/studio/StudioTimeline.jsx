...
// Track definitions — minimal color, mostly grayscale
const TRACK_DEFS = [
...
export default function StudioTimeline({
  ...
  // Clip colors — subtle tints so clips are distinguishable but not garish
  const getClipStyle = (type, isSelected) => {
    if (isSelected) {
      if (type === 'text') return 'bg-neutral-600 border-white ring-1 ring-white';
      if (type === 'audio') return 'bg-neutral-600 border-white ring-1 ring-white';
      return 'bg-neutral-600 border-white ring-1 ring-white';
    }
    if (type === 'text') return 'bg-neutral-700/80 border-neutral-500 hover:border-neutral-400';
    if (type === 'audio') return 'bg-neutral-700/60 border-neutral-500 hover:border-neutral-400';
    return 'bg-neutral-700 border-neutral-500 hover:border-neutral-400';
  };

  // Clip left accent stripe color (subtle type indicator)
  const getAccentColor = (type) => {
    if (type === 'text') return 'bg-neutral-400';
    if (type === 'audio') return 'bg-neutral-400';
    return 'bg-white';
  };

  const getIcon = (type) => {
    if (type === 'text') return <Type className="w-3 h-3 text-neutral-300" />;
    if (type === 'audio') return <Music className="w-3 h-3 text-neutral-300" />;
    return <VideoIcon className="w-3 h-3 text-neutral-300" />;
  };

  return (
    <div className="w-full h-full flex flex-col bg-black select-none">
      {/* Transport Bar */}
      <div className="flex items-center gap-1.5 px-2 py-1 bg-neutral-900 border-b border-neutral-800 shrink-0">
        {/* Playback */}
        ...
        {/* Timecode */}
        <div className="px-2 py-0.5 rounded bg-black border border-neutral-700 mx-1">
          <span className="text-xs font-mono text-white tabular-nums">{formatTime(currentTime)}</span>
        </div>
        ...
        <div className="flex items-center gap-0.5 ml-auto">
          ...
          <span className="text-[10px] text-neutral-500 font-mono w-7 text-center">{zoom}x</span>
        ...
        {hoverFrame !== null && (
          <span className="text-[10px] text-neutral-500 font-mono ml-1">{formatTimeShort(hoverFrame)}</span>
        )}
      </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* Track Labels */}
        <div className="flex flex-col shrink-0 bg-neutral-900 border-r border-neutral-800" style={{ width: LABEL_WIDTH }}>
          ...
          {TRACK_DEFS.map((track) => {
            ...
            return (
              <div key={track.id} className="flex items-center justify-between px-1.5 border-b border-neutral-800/50 group" style={{ height: TRACK_HEIGHT }}>
                <div className="flex items-center gap-1">
                  <TIcon className="w-2.5 h-2.5 text-neutral-500" />
                  <span className="text-[10px] font-semibold text-neutral-400">{track.label}</span>
                </div>
                <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => toggleTrackLock(track.id)} className="p-0.5 hover:bg-neutral-700 rounded" title={state.locked ? 'Unlock' : 'Lock'}>
                    {state.locked ? <Lock className="w-2.5 h-2.5 text-white" /> : <Unlock className="w-2.5 h-2.5 text-neutral-600" />}
                  </button>
                  <button onClick={() => toggleTrackVisibility(track.id)} className="p-0.5 hover:bg-neutral-700 rounded" title={state.visible ? 'Hide' : 'Show'}>
                    {state.visible ? <Eye className="w-2.5 h-2.5 text-neutral-600" /> : <EyeOff className="w-2.5 h-2.5 text-neutral-700" />}
                  </button>
                </div>
              </div>
            );
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto relative" onMouseMove={handleTimelineMouseMove} onMouseLeave={() => setHoverFrame(null)}>
          ...
          >
            {/* Ruler */}
            <div className="sticky top-0 z-30 bg-neutral-900/95 backdrop-blur-sm border-b border-neutral-700" style={{ height: RULER_HEIGHT }}>
              {rulerTicks.map((tick) => (
                <div key={tick.frame} className="absolute top-0" style={{ left: `${tick.x}px` }}>
                  <div
                    className={`w-px ${tick.isMajor ? 'bg-neutral-600' : 'bg-neutral-800'}`}
                    style={{ height: tick.isMajor ? RULER_HEIGHT : RULER_HEIGHT * 0.4, marginTop: tick.isMajor ? 0 : RULER_HEIGHT * 0.6 }}
                  />
                  {tick.isMajor && (
                    <span className="absolute top-0.5 left-1 text-[9px] text-neutral-500 font-mono whitespace-nowrap">
            ...
            {TRACK_DEFS.map((track, i) => {
              ...
              return (
                <div
                  key={track.id}
                  className={`absolute w-full border-b border-neutral-900 ${
                    state.locked ? 'bg-neutral-900/60' : i % 2 === 0 ? 'bg-neutral-950' : 'bg-black'
            ...
            {items.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className={`absolute rounded border overflow-hidden cursor-move transition-shadow ${getClipStyle(item.type, isSelected)} ${isSelected ? 'z-40' : 'z-10'} ${state.locked ? 'cursor-not-allowed opacity-60' : ''}`}
                ...
                >
                  ...
                  {/* Left resize handle */}
                  <div className="absolute left-0 top-0 bottom-0 w-2 hover:bg-white/10 cursor-w-resize z-20" onMouseDown={(e) => handleMouseDown(e, item, 'resize-l')} />
                  ...
                  <div className="flex items-center gap-1.5 pl-3 pr-2 h-full pointer-events-none overflow-hidden">
                    ...
                    {widthPx > 80 && (
                      <span className="text-[9px] text-neutral-500 ml-auto shrink-0">{formatTimeShort(item.durationInFrames || 150)}</span>
                    )}
                  </div>
                  ...
                  {item.type === 'audio' && widthPx > 40 && (
                    <div className="absolute inset-0 flex items-center px-3 pointer-events-none opacity-20">
                      {Array.from({ length: Math.min(Math.floor(widthPx / 3), 80) }).map((_, i) => (
                        <div key={i} className="w-0.5 mx-px bg-white rounded-full" style={{ height: `${30 + Math.sin(i * 0.7) * 40 + Math.random() * 20}%` }} />
                      ))}
                    </div>
                  )}

                  {/* Right resize handle */}
                  <div className="absolute right-0 top-0 bottom-0 w-2 hover:bg-white/10 cursor-e-resize z-20" onMouseDown={(e) => handleMouseDown(e, item, 'resize-r')} />
                </div>
              );
            })}
            ...
            <div className="absolute top-0 z-50 pointer-events-none" style={{ left: `${currentTime * pixelsPerFrame}px`, height: `${RULER_HEIGHT + TRACK_DEFS.length * TRACK_HEIGHT}px` }}>
              <div className="relative">
                <div className="absolute -left-[5px] top-0 w-0 h-0 border-l-[5px] border-r-[5px] border-t-[8px] border-l-transparent border-r-transparent border-t-white" />
                <div className="absolute left-0 top-[8px] w-px bg-white/70" style={{ height: `${TRACK_DEFS.length * TRACK_HEIGHT + RULER_HEIGHT - 8}px` }} />
              </div>
            </div>

            {/* Hover line */}
            {hoverFrame !== null && !dragging && (
              <div className="absolute top-0 w-px bg-neutral-700/50 pointer-events-none z-20" style={{ left: `${hoverFrame * pixelsPerFrame}px`, height: `${RULER_HEIGHT + TRACK_DEFS.length * TRACK_HEIGHT}px` }} />
            )}
          </div>
        </div>
      </div>
