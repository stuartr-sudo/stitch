...
- Migrations are loose SQL files at project root (`supabase-migration.sql`, `supabase-migration-v2.sql` through `v8`, plus `-tags`, `-storyboard-presets`, `-shorts-overhaul`, `-linkedin`, `-app-source`, `-library-metadata`, `-proposal-media`, `-storyboard-system`, `-carousel`, `-carousel-video`, `-ads`) and in `api/migrations/`. Not managed by Supabase CLI.
