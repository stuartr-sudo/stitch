...
**Shorts Pipeline** (`api/lib/shortsPipeline.js`): TTS-first Shorts creation with dual generation modes. Pipeline: narrative (words-only) → TTS voiceover → Whisper timestamps → block alignment (snap to model-valid durations) → scene direction (N+1 keyframe prompts) → image + video generation → music → assembly → captions → draft. Two generation paths auto-selected by video model:

- **V3 FLF** (Veo 3.1, Kling V3, Kling O3): Sequential I2I keyframe chain — scene 1 first frame (T2I or starting_image), then each subsequent keyframe generated via I2I (`nano-banana-2/edit`) from the previous one. Once all N+1 keyframes exist, all N first-last-frame videos fire in parallel.
- **V2 Extract** (Wan 2.5, Kling 2.0, Hailuo, etc.): Scene 1 image (T2I) → animate → extract last frame → use as next scene's image → animate → repeat. Fully sequential.

Key modules: `narrativeGenerator.js` (Stage 1: words-only script), `sceneDirector.js` (Stage 2: N+1 keyframe prompts after TTS timing known), `visualPromptComposer.js` (deterministic prompt assembly, no LLM), `getWordTimestamps.js` (Whisper via FAL), `blockAligner.js` (snap to model durations), `assemblyValidator.js` (pre-assembly timing check), `topicDiscovery.js` (GPT-powered topic suggestions). Per-scene checkpointing to `jobs.step_results`. Draft metadata includes `pipeline_version: 'v3'` and `generation_mode: 'v3_flf'|'v2_extract'`. Entry point: `api/campaigns/create.js`. Topic discovery API: `POST /api/shorts/discover-topics`.

**Script Generator** (`api/lib/scriptGenerator.js`): Wrapper around `narrativeGenerator.js` for backward compatibility. `generateScript()` calls `generateNarrative()` (words-only output: narration segments, overlay text, scene labels — NO visual prompts) and maps to the old format with empty `visual_prompt`/`motion_prompt` fields. Visual prompts are generated later by `sceneDirector.js` after TTS timing is known. Framework scene count comes from `sceneStructure`. Legacy fallback: 15s = 3 scenes, 30s = 3, 45s = 4, 60s = 5, 90s = 7.
...
**Storyboard Tool** (`api/storyboard/projects.js` + `api/storyboard/generate.js` + `StoryboardEditor.jsx` + `StoryboardList.jsx`): Persistent, database-backed frame-level video planning. Unlike the Storyboard Planner wizard (modal, ephemeral), this is a full page at `/storyboards` and `/storyboards/:id`. Users create a storyboard (name, length, frame interval) → N empty frames created in DB (`storyboard_frames` table) → generate script (Stage 1 narrative + Stage 2 visual director via `storyboardNarrativeGenerator.js` + `storyboardVisualDirector.js`) → generate preview images → inline edit/lock frames → export PDF → share public review link (`/api/storyboard/review/:token`, no auth) → approve for production. Frame count = `desired_length / frame_interval`. DB tables: `storyboards` + `storyboard_frames`. Migration: `migration-storyboard-system.sql`. CRUD API uses path-based routing in `projects.js` — the `app.all('/api/storyboard/projects*')` catch-all handles list/get/create/update/delete/frames/share. Script + preview generation routes registered BEFORE the catch-all in server.js.
...
- `/storyboards` and `/storyboards/:id` are the Storyboard Tool pages (persistent, DB-backed). The Storyboard Planner wizard (`StoryboardPlannerWizard.jsx`) is a separate modal opened from the Video Ad Creator — don't confuse the two systems.
