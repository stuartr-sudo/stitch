...
**Visual Styles** (`api/lib/visualStyles.js`): 13 visual style presets for the Shorts pipeline image generation. Frontend mirror: `src/lib/visualStylePresets.js`. Note: the shared StyleGrid uses `src/lib/stylePresets.js` (~118 styles) which is a separate, larger preset system — see Style Presets below.
...
**LinkedIn Posting Tool** (`src/pages/LinkedInPage.jsx` + `api/linkedin/` + `src/components/linkedin/`): Internal LinkedIn content creation tool at `/linkedin`. Two-panel layout: topic queue (left) + post feed (right). Discover topics via SerpAPI Google News search or paste URLs → score with GPT-4.1 mini → generate 3 post variations per topic (Contrarian, Story-Led, Data/Stat Punch styles) with anti-slop rules → review/edit → compose branded images via `sharp` SVG overlay → publish to LinkedIn API v2. 11 API endpoints under `/api/linkedin/*` (config CRUD, search, add-topic, topics list, update-topic, generate-posts, posts list, update-post, regenerate-post, publish). Database: `linkedin_config` (per-user settings, LinkedIn token, Exa key), `linkedin_topics` (discovery queue with scores), `linkedin_posts` (generated variations with images). Migration: `supabase-migration-linkedin.sql`. Image composition: `api/lib/composeImage.js` (9-layer SVG: gradient + orb + noise + quote + pills + logo) + `api/lib/colorTemplates.js` (6 auto-rotating color templates). Scoring: `api/lib/newsjackScorer.js` (GPT-4.1 mini structured output + Exa API content extraction). Publishing: `api/lib/linkedinPublisher.js` (LinkedIn API v2 with image upload). Fonts required in Docker: `fonts-dejavu-core`, `fonts-liberation`. Cost categories: `openai` (scoring/generation/excerpt), `fal` (base image), `serpapi` (search), `exa` (content fetch).
...
- Migrations are loose SQL files at project root (`supabase-migration-v*.sql`, `supabase-migration-tags.sql`, `supabase-migration-storyboard-presets.sql`, `supabase-migration-shorts-overhaul.sql`) and in `api/migrations/`, not managed by Supabase CLI.
...
- Imagineer has its own `STYLE_PROMPTS` map (~81 styles) in `api/imagineer/generate.js` — these are separate from the 13 visual styles in `api/lib/visualStyles.js` (which are for Shorts only). Don't confuse the two.
...
- Pushing to `main` triggers GitHub Actions auto-deploy, which restarts the Fly machine and KILLS any running pipeline mid-generation. This caused multiple failed generations. ALWAYS: `fly deploy` first, verify it's live, THEN `git push`. Never push during active generation.
