...
- Music generation uses MiniMax v2 only (`fal-ai/minimax-music/v2`). Beatoven has been completely removed from the codebase. MiniMax doesn't accept a duration parameter — it generates fixed-length tracks (~60s). The FFmpeg compose `duration` param trims output to match video length. Music must ALWAYS be instrumental (`lyrics_prompt: '[Instrumental]'`), never lyrics.
