...
**Platform OAuth**: Auth endpoints require `authenticateToken`; callbacks do NOT (OAuth redirects). All use `oauth_nonces` for CSRF. LinkedIn (`api/linkedin/oauth-auth.js` + `oauth-callback.js`), Meta (`api/meta/auth.js` + `callback.js` — single flow for both IG + FB), TikTok (`api/tiktok/auth.js` + `callback.js` — PKCE required).
