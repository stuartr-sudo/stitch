...
- FAL CDN URLs (`v3b.fal.media`, `fal.media`) are temporary and expire within hours. Always upload generated media to Supabase storage via `uploadUrlToSupabase()` or the `/api/library/save` endpoint before storing URLs in the database or displaying them in production pages. The Shorts pipeline does this automatically via `animateImageV2`.
